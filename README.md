...
- [Solace PubSub+ Event Broker Rootless](#solace-pubsub-event-broker-rootless)
...
By `rootless`, I mean a) invoking the container engine without root priviledge and b) running an app in the container as a non-root user. Docker supports the second part - mapping users inside containers to an unpriviledged range in the host with `--userns-remap`, providing better security for when containers contend for the same external resources.
...
![namespaces](images/k3s-namespaces.png)
...
[Solace PubSub+ Event Broker]: https://www.solace.dev/
...
[Experimenting with Rootless Docker]: https://www.docker.com/blog/experimenting-with-rootless-docker/